...
# Function to load the dataset from a .txt file
...
# dataset_names = ['pek']
task_names = ['loc_pre', 'classify']
embed_names = ['skipgram', 'poi2vec', 'teaser', 'hier', 'tale', 'ctle']
device = 'cuda:0'
...
for dataset_name in dataset_names:
    for embed_name in embed_names:  
        ...
        print("Split days:", split_days)
        print("Dataset days:", raw_df['datetime'].dt.day.unique())

        dataset = Dataset(raw_df, coor_df, split_days)

        # dataset_name = 'Brightkite_totalCheckins'  # or any other dataset name if applicable
        # # dataset_name = 'Gowalla_totalCheckins'  # or any other dataset name if applicable
        # file_path = os.path.join('data', '{}.txt'.format(dataset_name))
        # raw_df = load_dataset(file_path)
        # print(raw_df.head())
        # user_trajectory_counts = raw_df['user'].value_counts()
        # users_with_at_least_4_trajectories = user_trajectory_counts[user_trajectory_counts >= 4].index
        # sampled_users = random.sample(list(users_with_at_least_4_trajectories), 1000)
        # raw_df = raw_df[raw_df['user'].isin(sampled_users)]
        # split_days = [list(range(16, 23)), list(range(23, 25)), list(range(25, 27))]
        # if dataset_name == 'pek':
        #     split_days = [list(range(9, 12)), [12], [13]]
        # print("Split days:", split_days)
        # print("Dataset days:", raw_df['check-in time'].dt.day.unique())
        # dataset = Dataset(raw_df, split_days)
        ...
        if task_name == 'loc_pre':
            print("LOCATION POINTS: ", dataset.num_loc)
            print("USER POINTS: ", dataset.num_user)
            pre_model_name = param.get('pre_model_name', 'stlstm')
            ...
            if pre_model_name == 'mc':
                ...
                loc_prediction(dataset, pre_model, pre_len=pre_len, num_epoch=task_epoch,
                            batch_size=64, device=device, embed_name=embed_name, task_name=task_name, pre_model_name=pre_model_name, patience = 10)
